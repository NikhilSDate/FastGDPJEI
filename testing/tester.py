...
def f1_score(ground_truth_interpretation, ground_truth_lines, ground_truth_circles, predicted_interpretation, predicted_lines, predicted_circles):
    ...
    for id1, line in ground_truth_lines.items():
        for id2, predicted_line in predicted_lines.items():
            if id2 not in matched_lines and close_enough(hesse_normal_form((line[0][0], line[0][1], line[1][0], line[1][1])), predicted_line, image_size=(200, 200)):
...
for file_name, interpretation, lines, circles in parse_annotations():
    ...
    if interpretation.total_properties()>0:
        ...
        predicted_interpretation, predicted_lines, predicted_circles = parse_diagran(diagram_image)
        print(f1_score(interpretation, lines, circles, predicted_interpretation, predicted_lines, predicted_circles))
